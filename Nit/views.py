# Nit/views.py - COMPLETE FIXED VERSION
...
def home_view(request):
    context = {
        ...
        'recent_surveys': Assessment.objects.order_by('-created_at')[:5],
    }
...
def about_view(request):
    return render(request, 'Nit/about.html')


@login_required
def wards_view(request):
    wards = Ward.objects.filter(is_active=True)
    return render(request, 'Nit/wards.html', {'wards': wards})
...
def map_view(request):
    ...
    try:
        # Get ALL buildings from ALL corporations
        ...
        for building in all_buildings:
            try:
                # CRITICAL FIX: Convert Polygon to GeoJSON dict
                if building.geometry:
                    # Method 1: Using geojson property
                    if hasattr(building.geometry, 'geojson'):
                        geometry_json = building.geometry.geojson
                        geometry = json.loads(geometry_json)
                    # Method 2: If it's already a dict
                    elif isinstance(building.geometry, dict):
                        geometry = building.geometry
                    else:
                        # Method 3: Convert using GEOSGeometry
                        geometry = json.loads(building.geometry.wkt)  # Try WKT
                else:
                    continue
                ...
                logger.error(f"Error processing building {building.id}: {str(e)}")
                continue
        
        # Calculate total stats from ALL corporations
        total_features = len(features)
        total_area = all_buildings.aggregate(Sum('area'))['area__sum'] or 0
        
        # Get corporation-specific counts
        corporation_stats = []
...
def get_buildings_geojson(request):
    ...
    try:
        ...
        logger.error(f"Error in get_buildings_geojson: {str(e)}")
        return JsonResponse({
            "type": "FeatureCollection",
            "features": []
        }, safe=False)
...
@admin_required
def upload_corporation_geojson(request, corporation_id):
    """Upload GeoJSON for a corporation"""
    if request.method != 'POST':
        return JsonResponse({'error': 'Method not allowed'}, status=405)
    ...
    if not geojson_file:
        messages.error(request, 'No GeoJSON file uploaded')
        return redirect('corporation_detail', corporation_id=corporation.id)
    ...
    try:
        ...
        if not features:
            messages.warning(request, 'No features found in GeoJSON file')
            return redirect('corporation_detail', corporation_id=corporation.id)
        ...
        for i, feature in enumerate(features):
            try:
                ...
                try:
                    # Check if geometry is already a dict or string
                    if isinstance(geometry, dict):
                        ...
                        geom = GEOSGeometry(json.dumps(geom_dict), srid=3857)
                    elif isinstance(geometry, str):
                        # If it's a WKT string
                        geom = GEOSGeometry(geometry, srid=3857)
                    else:
                        error_count += 1
                        error_messages.append(f"Feature {i}: Invalid geometry type")
                        continue
                        
                except Exception as e:
                    error_count += 1
                    error_messages.append(f"Feature {i}: Geometry error - {str(e)}")
                    continue
                
                # Get GIS ID
                gis_id = properties.get('gis_id') or properties.get('GIS_ID')
                if not gis_id:
                    gis_id = f"B-{str(i+1).zfill(4)}"
                ...
                building = Building.objects.create(
                    corporation=corporation,
                    gis_id=gis_id,
                    building_number=properties.get('building_number') or gis_id,
                    ...
                    area=area,
                    floors=int(properties.get('floors', 1)),
                    year_built=properties.get('year_built'),
                    owner_name=properties.get('owner_name', 'Unknown'),
        ...
        if created_count > 0:
            messages.success(request, f'Successfully created {created_count} buildings!')
        
        if error_count > 0:
            messages.warning(request, f'{error_count} features failed to import. Check logs for details.')
        ...
        messages.error(request, f'Invalid GeoJSON file: {str(e)}')
        return redirect('corporation_detail', corporation_id=corporation.id)
    except Exception as e:
...
def buildings_geojson(request, corporation_id=None):
    ...
    if corporation_id:
        # Return buildings for a specific corporation
        buildings = Building.objects.filter(corporation_id=corporation_id, geometry__isnull=False)
    else:
        # Return ALL buildings (for the common map view)
...
def process_geojson(geo_file, data, geo_type):
    try:
        ...
        table_name = getattr(data, geo_type)
        if not table_name:
            return
        ...
        with connection.cursor() as cursor:
            for i, feature in enumerate(features):
                ...
                if geo_type == 'polygon':
                    ...
                    if coordinates and isinstance(coordinates[0], list) and coordinates[0]:
                        if isinstance(coordinates[0][0], list) and len(coordinates[0][0]) > 0:
                    ...
                    cursor.execute(f"""
                        INSERT INTO `{table_name}` (gisid, type, coordinates, created_at, updated_at)
                        VALUES (%s, %s, %s, NOW(), NOW())
                    """, [
                        str(i),
                    ...
                    """, [
                        str(i),
                        geometry.get('type', geo_type.capitalize()),
                        json.dumps(geometry.get('coordinates', []))
                    ])
    except Exception as e:
        print(f"Error processing {geo_type}: {e}")
